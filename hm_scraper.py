...
async def _auto_scroll(page: Page, max_rounds: int = 60, pause_ms: int = 700):
    ...
    for i in range(max_rounds):
        ...
        cur_card_count = await page.locator(CARD_SELECTOR).count()
        
        # Verifica se altura e número de cards estabilizaram
        if cur_h <= prev_h and cur_card_count <= prev_card_count:
            stable += 1
        else:
            stable = 0
        ...
        # Scroll mais agressivo para garantir que todos os cards sejam carregados
        await page.mouse.wheel(0, 3000)
        await page.wait_for_timeout(pause_ms)
        
        # Scroll adicional para garantir que elementos sejam renderizados
        await page.evaluate("window.scrollBy(0, 500)")
        await page.wait_for_timeout(200)
        
        prev_h = cur_h
        prev_card_count = cur_card_count
    log.info("Auto-scroll finalizado (altura=%s, cards=%s, iterações=%s).", prev_h, prev_card_count, i + 1)
...
async def _load_more(page: Page, max_clicks: int = 8):
    ...
    for _ in range(max_clicks):
        try:
            # Tenta múltiplos seletores para botão "carregar mais"
            btn_selectors = [
                ...
                try:
                    ...
                    if await btn.count() > 0:
                        ...
                        if await first_btn.is_visible():
                            # Scroll até o botão antes de clicar
                            await first_btn.scroll_into_view_if_needed()
                            await page.wait_for_timeout(300)
                            await first_btn.click()
                            log.info("Clicado em 'Carregar mais' (seletor: %s).", selector)
            ...
            break
    if clicks_done > 0:
        log.info("Total de cliques em 'Carregar mais': %d", clicks_done)
...
() => {
  ...
  // Função para extrair chave única do produto
  const getProductKey = (url) => {
    try {
      const path = new URL(url, window.location.origin).pathname;
      const digits = path.match(/\d{6,}/);
  ...
  for (const card of cards) {
    ...
    // Ignora links de seleção de cor
    if (anchor.closest('[data-color-selector-hex]')) continue;
    if ((anchor.dataset || {}).testid === 'color-block') continue;
    
    const href = anchor.getAttribute('href') || anchor.href || '';
    if (!href) continue;
    
    // Normaliza href relativo para absoluto
    let fullHref = href;
    if (href.startsWith('/')) {
      fullHref = window.location.origin + href;
    } else if (!href.startsWith('http')) {
      fullHref = window.location.origin + '/' + href.replace(/^\//, '');
    }
    
    if (!PDP.test(fullHref)) continue;
    
    // Usa chave única para evitar duplicatas
    const key = getProductKey(fullHref);
...
async def _ensure_cards_loaded(page: Page, category_url: str, page_number: int) -> bool:
    for attempt in range(1, PAGE_RETRIES + 1):
        try:
            ...
            if count > 0:
                # Aguarda um pouco mais para garantir que mais cards sejam carregados
                await page.wait_for_timeout(2_000)
                # Faz um scroll inicial para disparar lazy loading
                await page.evaluate("window.scrollTo(0, document.body.scrollHeight / 2)")
                await page.wait_for_timeout(1_500)
                # Verifica novamente após scroll
                final_count = await page.locator(CARD_SELECTOR).count()
                log.info("Cards carregados na página %d: %d (após scroll: %d)", page_number, count, final_count)
                return True
            log.warning(
...
async def discover_pdp_urls(page: Page, limit: Optional[int], category_url: str) -> List[str]:
    ...
    # Coleta inicial de cards visíveis
    card_urls = await _collect_card_urls(page)
    await add_urls(card_urls, "cards-initial")
    
    # Fallback para DOM completo se nenhum card foi capturado
    if not found:
        await add_from_ctx(page, "document-initial")
    ...
    if USE_FRAME_SCAN and not quota_reached():
        for fr in page.frames:
            if fr == page.main_frame:
                continue
            try:
                await add_from_ctx(fr, f"frame-initial:{fr.url}")
            except Exception:
                continue
    ...
    # Múltiplas rodadas de scroll e load more para garantir que todos os produtos sejam carregados
    max_scroll_rounds = 5
    for round_num in range(max_scroll_rounds):
        ...
        await _auto_scroll(page, max_rounds=20, pause_ms=500)
        await wait_for_candidates()
        
        # Recoleta após scroll
        card_urls_after = await _collect_card_urls(page)
        await add_urls(card_urls_after, f"cards-scroll-round{round_num+1}")
        
        if quota_reached():
            break
            
        # Se não encontrou novos produtos nesta rodada, tenta fallback
        if len(found) == prev_count:
            await add_from_ctx(page, f"document-scroll-round{round_num+1}")
            ...
            if USE_FRAME_SCAN and not quota_reached():
                for fr in page.frames:
                    if fr == page.main_frame:
                        continue
                    try:
                        await add_from_ctx(fr, f"frame-scroll-round{round_num+1}:{fr.url}")
                    except Exception:
                        continue
            
            # Se ainda não encontrou novos produtos após fallback, pode ser que não haja mais
            if len(found) == prev_count:
                log.info("Nenhum novo produto encontrado na rodada %d; finalizando coleta.", round_num + 1)
                break

    # Fallback por clique apenas se ainda não atingiu o limite e está configurado
    remaining = None if limit is None else max(limit - len(found), 0)
    if USE_CLICK_FALLBACK and limit is not None and remaining is not None and remaining > 0:
