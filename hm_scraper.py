...
# Pattern to match product URLs - updated to handle /slug/p format (p at end)
PDP_URL_RE = re.compile(r"(?:/p(?:[/?#]|$)|/(?:produto|product|productpage)\b)", re.I)
...
PRODUCT_HINT_SELECTORS = [
    ...
    "a[href$='/p']",  # New format: URLs ending in /p
    "a[href*='/produto']",
    "a[href*='/productpage']",
    "a[href*='/product/']",
    "[data-product-url]",
...
() => {
  ...
  //  - <a href> com /p ou /produto|product/ (URL pode terminar com /p)
  //  - atributos: data-href, data-url, data-product-url, data-link, onclick (com URL)
  //  - JSON-LD com url para produto
  const out = new Set();
  // Updated regex: matches /slug/p at end OR /produto|product|productpage/ in path
  const PDP = /(?:\/p(?:[?#]|$|")|\/(?:produto|product|productpage)\b)/i;
    ...
    try {
      ...
      root.querySelectorAll('[data-href],[data-url],[data-product-url],[data-link],[onclick]').forEach(el => {
        ...
        for (const k of attrs) {
          ...
          if (!v) continue;
          const str = String(v);
          // Match both /produto|product/... and .../p patterns
          const m = str.match(/['"]((?:https?:)?\/?[^'"]*(?:\/(?:produto|product)\/[^'"]+|\/p(?:[?#]|$|")))['"]/i);
