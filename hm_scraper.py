...
async def _auto_scroll(page: Page, max_rounds: int = 100, pause_ms: int = 700):
    prev_h = 0
    stable = 0
    for i in range(max_rounds):
        ...
        if cur_h <= prev_h:
            ...
            stable = 0
        if stable >= 5:  # Aumentado de 3 para 5 para garantir que todos os produtos sejam carregados
            break
...
async def _load_more(page: Page, max_clicks: int = 20):
    clicks = 0
    for _ in range(max_clicks):
        try:
            ...
            if await btn.count() > 0 and await btn.first.is_visible():
                await btn.first.click()
                clicks += 1
                log.info("Clicado em 'Carregar mais' (%d/%d).", clicks, max_clicks)
                await page.wait_for_timeout(2000)  # Aumentado para dar tempo de carregar
            else:
                break
        except Exception:
            break
    if clicks > 0:
        await page.wait_for_timeout(1500)  # Aguarda um pouco mais após os cliques
...
() => {
  ...
  for (const card of cards) {
    // Tenta múltiplos seletores para encontrar o link do produto
    let anchor =
    ...
    if (!anchor) {
      // Se não encontrou anchor direto, tenta buscar em shadow roots
      const shadowRoot = card.shadowRoot;
      if (shadowRoot) {
        anchor = shadowRoot.querySelector('a[href]');
      }
    }
    ...
    if (!PDP.test(normalizedHref) && !PDP.test(href)) continue;
    
    // Usa href normalizado para comparação
    const key = normalizedHref.split('?')[0]; // Remove query params para comparação
    if (seen.has(key)) continue;
    seen.add(key);
    out.push(normalizedHref);
  }
  
  console.log(`URLs de produtos encontradas: ${out.length}`);
  return out;
}
"""
    ...
    try:
        ...
        for u in urls:
            if not u:
                continue
            # Normaliza a URL
            nh = _normalize_href(u)
            if not PDP_URL_RE.search(nh):
                continue
            # Usa a URL normalizada para evitar duplicatas
            if nh not in seen:
                uniq.append(nh)
                seen.add(nh)
        
        # Conta cards visíveis para debug
        try:
            card_count = await page_or_frame.locator(CARD_SELECTOR).count()
            if card_count > len(uniq):
                log.warning(
                    "Encontrados %d cards mas apenas %d URLs coletadas. "
                    "Alguns cards podem não ter links válidos.",
                    card_count, len(uniq)
                )
        except Exception:
            pass
        ...
        log.warning("Erro ao coletar URLs dos cards: %s", e)
        return []
...
async def discover_pdp_urls(page: Page, limit: Optional[int], category_url: str) -> List[str]:
    ...
    # Coleta inicial via cards visíveis
    card_urls = await _collect_card_urls(page)
    await add_urls(card_urls, "cards-inicial")
    ...
    # Coleta iterativa: faz múltiplas rodadas de scroll/load até não encontrar mais produtos
    max_iterations = 15  # Aumentado para garantir que todos os produtos sejam carregados
    consecutive_no_new = 0
    max_consecutive_no_new = 2  # Para após 2 iterações sem novos produtos
    ...
    for iteration in range(max_iterations):
        ...
        # Conta cards visíveis antes da iteração
        try:
            card_count_before = await page.locator(CARD_SELECTOR).count()
        except Exception:
            card_count_before = 0
        
        log.info("Iteração %d/%d de coleta (produtos encontrados: %d, cards visíveis: %d)", 
                 iteration + 1, max_iterations, prev_count, card_count_before)
        
        # Tenta carregar mais produtos
        ...
        await wait_for_candidates()
        await page.wait_for_timeout(2000)  # Aumentado para dar mais tempo de carregamento
        
        # Conta cards após scroll/load
        try:
            card_count_after = await page.locator(CARD_SELECTOR).count()
            if card_count_after > card_count_before:
                log.info("Novos cards carregados: %d -> %d (+%d)", 
        ...
        # Coleta novos cards
        await add_urls(await _collect_card_urls(page), f"cards-iter{iteration+1}")
        
        # Também tenta coletar do DOM completo
        await add_from_ctx(page, f"document-iter{iteration+1}")
        
        # Verifica se encontrou novos produtos
        current_count = len(found)
        if current_count == prev_count:
            consecutive_no_new += 1
            log.info("Nenhum novo produto encontrado (consecutivos sem novos: %d/%d)", 
                    consecutive_no_new, max_consecutive_no_new)
            if consecutive_no_new >= max_consecutive_no_new:
                log.info("Finalizando coleta após %d iterações sem novos produtos.", 
                        consecutive_no_new)
                break
        else:
            consecutive_no_new = 0  # Reset contador se encontrou novos produtos
        
        if quota_reached():
            break

    # Fallback adicional: tenta coletar de frames após todas as iterações
    if USE_FRAME_SCAN and not quota_reached():
        for fr in page.frames:
            if fr == page.main_frame:
                continue
            try:
                await add_from_ctx(fr, f"frame-final:{fr.url}")
            except Exception:
                continue

    # Se ainda insuficiente e limit não é None, fallback por clique
