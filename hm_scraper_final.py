...
NUMERIC_ALLOWED = {str(n) for n in range(30, 56, 2)}
...
# ------------------ Enhanced discovery with GraphQL interception ------------------


async def discover_pdp_urls_graphql(page: Page, category_url: str) -> List[str]:
    """
    Discovers product URLs by intercepting GraphQL API responses.
    The site loads products via GraphQL with pagination (after=0, 36, 72, ...).
    We scroll to trigger these API calls and collect unique products.
    """
    ...
    try:
        ...
        log.warning("Cards not loaded initially")
    ...
    for i in range(150):  # Increased max rounds
        # Scroll
...
async def main():
    async with async_playwright() as p:
        ...
        for offset in range(PAGES_TO_SCAN):
            ...
            novos = await discover_pdp_urls_graphql(page, category_url)
